...
void RedBlackTree::deleteValue(NodeRB* node, int key) {
    ...
    //w wypadku gdy wezel ma obydwa dzieci funkcja znajduje minimalna wartosc prawego poddrzewa - nastepnika
    //wezla usuwanego, aby ulatwic usuniecie elementu
    else{
        pom2 = findMinimumNode(pom2->getChildRight());//nastepnik usuwanego wezla
...
void RedBlackTree::rotateLeft(NodeRB *node) {
    ...
    //zamienia lewe dziecko tego wezla na dziecko, którym wcześniej byl wezeł rotatowany dla swojego rodzica
    //lewe jesli był lewym etc.
...
void RedBlackTree::rotateRight(NodeRB *node) {
    ...
    //zamienia prawe dziecko tego wezla na dziecko, którym wcześniej byl wezeł rotatowany dla swojego rodzica
    //lewe jesli był lewym etc.
...
NodeRB* RedBlackTree::searchKey(int key){
    ...
    if (node == nullNode) cout<<"Match not found\n";
